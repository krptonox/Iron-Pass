...
describe("IronPass Custom Errors", () => {
  ...
  it("should create InvalidNormalizationFormError correctly", () => {
    const error = new InvalidNormalizationFormError("Invalid normalization");

    expect(error).toBeInstanceOf(Error);
    expect(error.name).toBe("InvalidNormalizationFormError");
    expect(error.message).toBe("Invalid normalization");
  });
});

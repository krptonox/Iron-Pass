...
import { verifyAndRehash } from '../src/core/verifyAndRehash.core.js';

import { MIN_ITERATIONS } from '../src/constants/defaults.js';
...
describe('verifyAndRehash', () => {
  it('should reject an incorrect password', async () => {
    ...
    const result = await verifyAndRehash('WrongPassword123', hashedPassword);
  ...
  it('should create a valid upgraded hash', async () => {
    ...
    const result = await verifyAndRehash(password, oldHash);

    expect(result.rehashed).toBe(true);

    const valid = await verifyPassword(password, result.hash);
  ...
  it('should generate a new hash for an outdated hash', async () => {
    ...
    expect(needsRehash(result.hash)).toBe(false);
  });
});

...
describe("IronPass Malformed Input Security", () => {
  describe("Malformed stored hashes", () => {
    ...
    ])("should safely reject malformed hash: %s", async (malformedHash) => {
      await expect(verifyPassword("IronPass@123", malformedHash)).rejects.toThrow(InvalidHashError);
    });

    it.each([null, undefined, 123, true, {}, [], Buffer.from("hash")])(
      "should reject non-string stored hash",
      async (malformedHash) => {
        await expect(verifyPassword("IronPass@123", malformedHash)).rejects.toThrow(
          InvalidHashError
        );
      }
    );
  ...
  describe("Malformed hash metadata", () => {
    it.each(["NaN", "Infinity", "-1", "0", "1.5", "abc"])(
      "should reject invalid iterations: %s",
      async (iterations) => {
        const malformedHash = `v1$pbkdf2$sha256$${iterations}$32$aabb$${"aa".repeat(32)}`;
    ...
    it.each(["NaN", "Infinity", "-1", "0", "1.5", "abc"])(
      ...
      async (keyLength) => {
        const malformedHash = `v1$pbkdf2$sha256$300000$${keyLength}$aabb$${"aa".repeat(32)}`;
  ...
  describe("Malformed password inputs", () => {
    it.each([null, undefined, 123, true, {}, [], Buffer.from("password")])(
    ...
    it("should reject an empty password", async () => {
      await expect(hash("")).rejects.toThrow(InvalidPasswordError);
    });
  });
  ...
  describe("Tampered valid hashes", () => {
    it("should reject a truncated valid hash", async () => {
      ...
      await expect(verifyPassword("IronPass@123", truncatedHash)).rejects.toThrow(InvalidHashError);
    });

    it("should reject a hash with injected separators", async () => {
      const hashedPassword = await hash("IronPass@123");

      const tamperedHash = hashedPassword + "$injected";

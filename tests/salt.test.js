...
import { DEFAULT_SALT_LENGTH } from '../src/constants/defaults.js';
...
describe('Salt Security', () => {
  ...
  it('should generate salt with correct hex length', () => {
    ...
    expect(salt.length).toBe(DEFAULT_SALT_LENGTH * 2);
  });
  ...
  it('should accept a valid hexadecimal salt', () => {
    expect(validateSalt('aabbccdd')).toBe(true);
  });

  it('should reject an empty salt', () => {
    expect(() => validateSalt('')).toThrow(InvalidSaltError);
  });

  it('should reject a non-string salt', () => {
    expect(() => validateSalt(12345)).toThrow(InvalidSaltError);
  });

  it('should reject invalid hexadecimal salt', () => {
    expect(() => validateSalt('NOTHEX')).toThrow(InvalidSaltError);
  });

  it('should reject odd-length hexadecimal salt', () => {
    expect(() => validateSalt('abc')).toThrow(InvalidSaltError);
  });
});
